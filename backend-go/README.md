...
- 可选且互斥的 `receiverUsername` / `groupID` 决定公共、私聊或群文件消息
...
- 头像和个人背景使用独立 multipart 接口，不会创建聊天消息
- 仅允许 JPEG/PNG，单文件最大 5 MiB
- 文件保存在 `UPLOAD_DIR/profile-media`，替换后清理旧的本地托管图片
- 图片地址公开读取；资料内容和修改接口仍要求登录
...
- 文件上传下载 HTTP 接口
- 通过上传触发的文件消息实时事件
- 群文件成员权限、头像/个人背景上传与 `user.profile.changed`
- 同账号多个 WebSocket 连接的首上线/末离线语义
