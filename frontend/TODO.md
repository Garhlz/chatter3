...
- [x] Tauri 基础桌面能力：
  ...
  - [x] Tauri 环境下迁移旧 localStorage 偏好值到桌面 store
  - [x] JWT 迁移到系统凭据库，旧 Tauri store/localStorage token 启动时迁移并清理
  - [x] 本地聊天记录快照缓存：按用户持久化会话列表、最近消息、分页 cursor 与滚动位置
  - [x] 打开文件与 URL
...
- [x] 启动时优先恢复本地聊天记录快照，再用远端历史覆盖公共大厅与在线态
- [ ] 继续收口 `desktop.ts`，把文件选择、下载、打开文件、打开目录统一进桌面能力抽象层
- [x] 实现 Tauri 单实例保护：重复启动时激活已有窗口
- [ ] 为 Tauri 环境补齐原生文件读取桥接，再把上传入口切回原生文件选择
- [x] 明确基础通知策略：仅非当前会话且窗口未聚焦/不可见时通知
- [ ] 补齐通知点击跳转对应会话
- [x] 补托盘菜单动作：显示主窗口、重连、退出
- [x] 把窗口聚焦/隐藏/恢复事件与前端状态联动，用于优化未读和通知策略
...
- [x] 系统级安全存储 token
  ...
  - [x] macOS Keychain
  - [x] Linux Secret Service / libsecret
- [ ] 文件下载桌面体验闭环
  - [ ] 选择保存位置
  - [ ] 下载进度
  - [ ] 下载完成后打开文件
