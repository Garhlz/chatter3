...
- 建群、加人和移除成员通过 `group.changed` 自动同步到相关在线成员
- 当前用户不会生成“和自己私聊”的会话，旧缓存中的自聊也会在恢复时清理
- 用户资料查看、本人资料编辑、头像与个人背景上传

### 当前 UI 与交互

- 原生桌面感双栏布局：左侧会话导航，右侧消息优先的聊天主区域
- 聊天标题栏只保留身份、异常连接状态和少量会话操作，开发面板移入设置入口
- 页面级 Toast 与登录失效横幅，正常连接状态不反复弹出提示
- 登录页使用桌面欢迎区 + 单表单模式，登录 / 注册通过切换进入
- 会话搜索、摘要、时间、未读数、在线状态、资料入口与建群弹窗
- 私聊支持显式创建“空会话壳”，资料弹窗可直接发起聊天
- 消息按日期、发送者和五分钟窗口分组；自己的消息靠右，其他人的消息靠左
- 文件气泡、鉴权图片预览、保存位置选择、发送失败重试与新消息提示
- 多行消息输入框：`Enter` 发送，`Shift+Enter` 换行，并保护中文输入法组合状态
- 草稿与滚动位置按会话保存，后台刷新不会改变用户当前所在会话
- 用户资料弹窗：查看身份、在线状态、注册时间；查看他人时可直接发起聊天
- 资料弹窗使用个人空间式摘要展示头像、自我介绍和本人私有信息；设置弹窗支持退出登录
- 退出时先关闭 realtime 并清空内存身份；即使系统凭据库删除失败也不会停留在半登录状态，失败原因会显示在登录页
- 群资料使用右侧抽屉按需展示群名、成员数、创建者、当前权限和管理动作
- 群成员移除的应用内确认区
- 窄桌面窗口使用覆盖式侧栏、遮罩关闭，并在打开会话后自动收起
...
- Tauri realtime 状态、重连事件已通过桌面桥接同步到前端状态层
- 同一账号可同时保持 Web 和 Tauri 连接，消息会投递到全部活跃连接
- 浏览器与 Tauri 使用一致的有界重连语义，连接稳定十秒后才清零连续失败次数
...
- JWT 持久化到系统凭据库
- SQLite 本地消息缓存
- 使用带 Bearer token 的 Rust HTTP client 读取文件，支持图片预览和原生保存对话框
- 桌面上传通过原生文件选择器取得路径，再由 Rust 读取和上传，避免 WebView 本地文件限制
...
- `PUT /api/v2/users/{username}/avatar|background`
  - Web 使用文件输入，Tauri 使用原生选择器和 Rust multipart client
  - 支持 JPEG/PNG；成功后更新个人空间并通过 `user.profile.changed` 同步缓存身份
...
- 群聊基本管理
- 用户资料读取与更新
- 头像和个人背景上传、`user.profile.changed` 实时同步
