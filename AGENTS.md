...
- 连续 bullet 之间只换行，不插入额外空白行；正文必须与实际提交内容一致，不写未完成事项。
...
- 代为提交时优先使用 heredoc、`git commit -F <message-file>` 或其他能精确保留多行格式的方式；不要用多个 `-m` 把每条 bullet 变成独立段落。
- 不要把个人编辑器配置、临时文件或无关生成物混入提交。
...
当前稳定契约包括：认证、历史消息、在线用户、同账号多 WebSocket 会话、公共/私聊/群聊文本与文件消息、文件下载权限、群详情/成员/历史的成员权限、事务化群成员写入，以及用户资料和资料图片读写。

当前尚不稳定或未实现：删除群组、已读回执、消息撤回和跨端已读状态同步。
...
- 设置弹窗负责语言、主题和开发者面板入口。
- 资料弹窗同时是查看身份、编辑本人资料与资料图片、发起私聊的入口。
